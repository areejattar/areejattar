...
function getMinPrice(p: Product): number {
  const vals = Object.values(p.prices || {}).filter((x): x is number => typeof x === "number");
  return vals.length ? Math.min(...vals) : (p.price ?? 0);
}
...
export default function OrderEnquiry() {
  ...
  const suggestions = products.filter(p => p.status === "published" && !items.some(i => i.slug === p.slug)).map(p => ({ p, min: getMinPrice(p) })).sort((a,b) => Math.abs(a.min - minRemaining) - Math.abs(b.min - minRemaining)).slice(0, 4).map(x => x.p);
